...
# metric tiles
tiles=[
 ("52%","earnings covered","(rest are in-line non-events → correct silence)",BLUE),
 ("66","catches price-only misses","7 thesis-only + 59 smart-money divergences",PURPLE),
 ("4.73x","alert concentration","on earnings vs a random day",RED),
...
for i,(big,lab,sub,c) in enumerate(tiles):
    ...
    ax.text(xx+tw/2,ty+11.4,big,ha="center",va="center",fontsize=19,fontweight="bold",color=c)
    ax.text(xx+tw/2,ty+6.6,lab,ha="center",va="center",fontsize=9.4,color=NAVY,fontweight="bold")
    ax.text(xx+tw/2,ty+3.0,sub,ha="center",va="center",fontsize=7.3,color=GREY,wrap=True)

# footer honesty
ax.add_line(plt.Line2D([6,94],[16,16],color=LINE,lw=1))
ax.text(6,12,"Event types: earnings (earnings-calendar) · insider/Form 4 (≥2 discretionary open-market buys, or a single ≥$10M Form 4) · price/proxy thesis (MSTR·COIN vs BTC).",fontsize=7.8,color=GREY)
ax.text(6,8.4,"Honest scope: recent ~24-month window; only sourceable event types (news / M&A / litigation / guidance endpoints unavailable). P0/P1 precision is a lower bound.",fontsize=7.8,color=GREY)
