...
|---|---|---|---|
| 1 | **The Skill** (single file + code) | [`portfolio-watch/`](portfolio-watch/) folder · packaged [`portfolio-watch-skill-v2.1.3.zip`](portfolio-watch-skill-v2.1.3.zip) | ✅ |
...
- Pinned creator's note inviting remix. **Public + community-discoverable** (Playbook
  v1.17.0): discovery tags `portfolio · monitoring · alerts · stocks · crypto · thesis ·
  risk · watchlist` set and **verified searchable** on Alva (`alva playbooks trending
  --tag monitoring` / keyword surface it). The skill `description` is also optimized with
  rich EN + 中文 trigger phrases so an agent routes "watch/monitor my stocks + alert me"
  to this skill.
