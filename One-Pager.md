...
- **Event-aligned evaluation** — not "did price continue," but "when a real event
  ...
  insider/Form 4 · thesis break): **52% earnings covered** (rest are in-line
  non-events it should skip), **66 catches a price-only tracker would miss**, **4.73×**
  alert concentration on earnings vs chance, ~6% duplicates, +1-day timing.
...
- **事件对齐评估**——不是"价格有没有延续",而是"真实事件发生时，它有没有变成有用的告警?"。在 **111 个真实事件**(财报 · insider/Form 4 · thesis 破裂)上:**52% 财报被覆盖**(其余是 in-line 非事件、本该跳过)、**66 个价格-only 会漏的事件被捕获**、告警在财报窗口的集中度是随机日的 **4.73 倍**、~6% 重复、+1 天时效。

![事件对齐评估——111 个真实事件](assets/fig7-event-aligned.png)
