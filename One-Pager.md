...
- **事件对齐评估**——不是"价格有没有延续",而是"真实事件发生时，它有没有变成有用的告警?"。在 **111 个真实事件**(财报 · insider/Form 4 · thesis 破裂)上:核心指标是**告警落在财报窗口的概率是随机日的 4.73 倍**(告警命中真实催化剂,而非噪音);另有 **66 个非价格事件由非价格维度呈现**(thesis + smart-money),价格-only 根本没有这个维度去表示它们。(52% 财报触发了告警——中性数字,另一半是 in-line 非事件;诚实边界:insider 是 coverage-by-construction、事件类型有限。)
