...
> 仍缺的:完整"事件级"(财报/新闻/filing 对齐)评估 —— 历史财报日在现有 endpoint 上不可靠获取,故标为 specced(见 Evaluation-Matrix.md)。本消融用**告警量 + 重复率**度量降噪,直接对应产品主张(注意力路由),而非价格预测。

## 9. 局限与下一步

- 次日延续是**纯价格**代理，不含成交量/新闻确认；接入后 precision 预计上行（产品实际就是这么叠加的）。
- 单标的样本小，结论以 cohort/全历史池化为准。
- 下一步：ground truth 换"事件对齐"（财报/新闻）交叉验证；按波动率/资产类型分层看 precision；把成交量确认作为第二特征进 P-R，量化确认层增益。

## 10. 可复现

- 脚本：`backtest/pw-backtest.js`（P-R 校准，WIN=400，种子 20260705）、`backtest/pw-ablation.js`（消融 + thesis，种子 20260706）
- 原始结果：`backtest/results-29symbols-400d.json`（R3）、`backtest/results-9symbols-400d.json`（R2）、`backtest/results-ablation.json`（消融）
