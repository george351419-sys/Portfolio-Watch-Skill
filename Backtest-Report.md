...
*局限(如实):* 近端窗口 + 仅财报(非新闻/filing);EPS surprise 幅度因 `eps_estimated` 接近 0 会失真,故不以 surprise 分层为准,只报召回 + 集中度。

**扩展到多事件类型:** 见 `Event-Aligned-Evaluation.md` —— 把评估锚定在 **111 个真实事件**(42 财报 · 62 insider/Form 4 · 7 thesis break)上,产出集中度 / 独有维度 / 重复率 / lead-lag 的完整汇总表。核心论据是 **4.73× 集中度**;另有 **66 个非价格事件由非价格维度呈现**(价格-only 无此维度)。诚实:52% 是中性数字非目标;insider 是 coverage-by-construction 非独立命中率。
