...
## 7b. 投资假设的动态闭环（Thesis-Linked，含 P0 越级）

前面所有信号都在回答"市场发生了什么"。更高一层的问题是"**当初买入的逻辑是否还成立**"。一次泛泛的 2σ 移动是*信息*；一次买入假设的破裂是*决策级*。所以 thesis 应是**一等监控对象**，其违背**越级 P0**。

### 7b.1 框架翻转（与 §1.4 的优雅对偶）

基础模型：**残差（个股独立移动）是信号，市场移动被上卷成噪音**（§3.1）。thesis 把它翻转——thesis 声明了"什么*本该*相关"，于是**对 thesis 基准的残差 = 假设违背**。同一套残差波动率数学，只是把基准从默认市场 SPY 换成 **thesis 指定的参照资产**（如 BTC）：

$$
\text{expected}_t = \beta_{\text{ref}}\, r_{\text{ref},t},\qquad
\text{divergence}_t = r_{i,t} - \text{expected}_t,\qquad
\boxed{\,V_t = \frac{|\text{divergence}_t|}{\sigma_{\text{resid}}}\,}
$$

$\beta_{\text{ref}}$、$\sigma_{\text{resid}}$ 由 $i$ 对参照资产的滚动 OLS 得到（§1.4 同款，换基准）。

### 7b.2 违背判据与越级

$$
\text{thesis break} \iff |z_{\text{ref}}| \ge 1.5 \ \wedge\ V_t \ge 2 \ \wedge\ \operatorname{sign}(\text{divergence}_t)\text{ 与 thesis 方向相悖}
$$

即"参照资产发生了显著移动，而持仓没有按放大关系跟上"。外加**慢速机制破裂**：滚动相关 $\rho_t(i,\text{ref})$ 数周内向 0 衰减，说明杠杆/代理关系在瓦解（即使没有单日剧烈背离）。确认破裂 → **直接 P0，绕过常规 σ 闸门**——在评分体系里等价于给 thesis 维度一个饱和的最高权重，因为"你持有的理由正在失效"本质上就是决策级。

### 7b.3 真实数据验证（MSTR = 杠杆 BTC）

`thesis-monitor.js` 在 5 年 MSTR/BTC 真实日线上扫描，最严重的一天：

> **2024-11-21：BTC +4.3%（1.5σ）→ thesis 预期 MSTR +6.5%（β=1.5），实际 −16.2%。背离 −22.6% = 对假设的 4.6σ 违背，60 日相关 0.65。** → P0："你把 MSTR 当 BTC 杠杆拿，可 BTC 涨的那天它跌了 16%，杠杆关系不成立了。"
...
| thesis | 不变量 | 违背 |
|---|---|---|
| 代理/杠杆（"X 是我的杠杆 Y"）| 高 $\beta_Y$、正 $\rho(X,Y)$ | 对 Y 基准的残差 $V$ 超阈 |
| 相对跑赢（"X 跑赢板块 Z"）| $X$ 持续跑赢 $Z$ | 持续相对跑输 |
| 对冲/分散（"X 对冲我的股票"）| 回撤中对组合负相关 | 选跌时相关翻正（不再对冲）|
| 基本面水平/催化（v2，需额外数据）| 命名条件成立 | 条件破裂 |
...
| **假设违背** | $V=\lvert r_i-\beta_{\text{ref}}r_{\text{ref}}\rvert/\sigma_{\text{resid}}$，破裂→**P0** | $\lvert z_{\text{ref}}\rvert{\ge}1.5,\ V{\ge}2$ |
