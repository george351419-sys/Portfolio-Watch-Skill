...
The headline: a price alert is **4.73× more likely** to sit on an earnings window than
on a random day — alerts track real catalysts, not price noise. Plus **66 non-price
events surfaced by non-price dimensions** (7 thesis-only breaks + 59 smart-money/insider
divergences) that a price-only tracker can't represent. (52% of earnings produced an
alert — a *neutral* rate: the other half are in-line non-events it should skip.) ~6%
duplicates, +1-day timing. Full method & honest scope (incl. that insider is
coverage-by-construction): [`Event-Aligned-Evaluation.md`](Event-Aligned-Evaluation.md).
...
- Alerts are delivered to **Discord + web push** (verified, status = sent). **Two distinct delivery paths, to be precise:** (a) *basic push* to any IM channel (Telegram/Slack/Discord) rides the **platform fanout** (`feed_alert_ready` → whatever `active_channel` the owner connects) — no code change; (b) Telegram **"Silent Update"** (edit one card in place) is *not* platform fanout — it goes through the **direct Telegram Bot API** in the feed (`deliverTelegram`: sendMessage → `editMessageText` silent updates → re-send on escalation), which is why it needs a BYOD `TELEGRAM_BOT_TOKEN` + `TELEGRAM_CHAT_ID`. The fusion **logic** is verified on the runtime; the silent-edit **delivery** isn't demoed here (no bot token on the demo account).
