...
- Alerts are delivered to **Discord + web push** (verified, status = sent). Telegram/Slack use the identical pipeline (`feed_alert_ready` → `active_channel`), no code change. Telegram "Silent Update" (edit one card in place) is now **wired in the feed** (`deliverTelegram`: sendMessage first, `editMessageText` for silent updates, re-send on escalation) — it activates when you set the `TELEGRAM_BOT_TOKEN` + `TELEGRAM_CHAT_ID` secrets (BYOD). Its fusion **logic** is verified on the runtime; end-to-end delivery isn't demoed here (no bot token on the demo account).
